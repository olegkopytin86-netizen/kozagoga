...
// Модульные роутеры (SRS Modules) — после express.json() и authenticate
...
app.post('/api/orders', requireAuth, rateLimit(getRateLimits().orders || 30), async (req, res) => {
  try {
    const { items, payment_method: rawPaymentMethod } = req.body
    const payment_method = sanitizeTextField(rawPaymentMethod)
...
app.get('/api/confirm-status/:orderId', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT id, payment_status, status, provider_status, updated_at FROM orders WHERE id = $1`,
      [orderId]
    )
    if (result.rows.length === 0) {
